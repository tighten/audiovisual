<x-app-layout>
    <div class="max-w-xl mx-auto mt-12 sm:px-6 lg:px-8 page-content">
        <h2 class="mt-4 text-xl font-bold">Resources I want to work from in expanding this</h2>
        <ul class="pl-6 list-disc">
            <li>My <a href="https://mattstauffer.com/blog/setting-up-your-webcam-lights-and-audio-for-remote-work-podcasting-videos-and-streaming/">original article</a> obv</li>
            <li>Everything from my buddy Michael Chan</li>
            <li>https://twitter.com/EricaJoy/status/1336920432411209728</li>
            <li>https://marco.org/podcasting-microphones</li>
            <li>https://www.hanselman.com/blog/good-better-best-creating-the-ultimate-remote-worker-webcam-setup-on-a-budget</li>
            <li>https://riverside.fm/blog/podcast-studio-setup</li>
            <li>https://riverside.fm/blog/podcast-microphone</li>
            <li>https://riverside.fm/blog/best-podcast-camera</li>
        </ul>
        <h2 class="mt-4 text-xl font-bold">Brainstorm features</h2>
        <ul class="pl-6 list-disc">
            <li>Listings of all mics/webcams/etc. I've tested and my thoughts, ordered by price</li>
            <li>"Get started doing ___" (podcasting, streaming, working from home) guides, each with "how much do you want to spend" type sliders/choosers</li>
            <li>Audio samples</li>
            <li>Video samples</li>
        </ul>
    </div>
</x-app-layout>
